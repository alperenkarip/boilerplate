...
- Core identity, orchestration principles, agent catalog: AGENTS.md
- Quality gates, security boundaries: .Codex/rules/moai/core/moai-constitution.md
- SPEC workflow phases, token budgets: .Codex/rules/moai/workflow/spec-workflow.md
- Development methodologies (DDD/TDD): .Codex/rules/moai/workflow/workflow-modes.md
- Agent definitions: See AGENTS.md Section 4. For agent creation, use builder-agent subagent.
- @MX tag rules and protocol: .Codex/rules/moai/workflow/mx-tag-protocol.md
...
For detailed orchestration: Read /Users/goos/MoAI/moai-adk-go/.Codex/skills/moai/workflows/security.md
...
- `ultrathink` keyword detected → Activate Codex's native extended reasoning (high effort mode). Do NOT invoke Sequential Thinking MCP. This is native Codex behavior with no MCP dependency.
