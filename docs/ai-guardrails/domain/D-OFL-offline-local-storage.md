...
kaynak-dokümanlar: ADR-019, ADR-020, 10, ADR-005
miras-tipi: yapısal
...
- Firestore offline persistence ile veri okuma/cache (canonical)
- Local storage operasyonları (MMKV, AsyncStorage, SecureStore)
- Zustand persistence entegrasyonu
- Offline callable mutation queue
...
### 1.4. Firestore Offline Persistence + Query Cache (Canonical)

16. [ZORUNLU] Server-synced domain verisinin offline cache'i Firestore offline persistence ile sağlanır — canonical ve koşulsuz aktif (mobilde `@react-native-firebase/firestore` otomatik; web'de `persistentLocalCache` / IndexedDB). Ayrı storage'a kopyalanmamalı
17. [ZORUNLU] Okuma offline'da Firestore SDK cache'inden servis edilir; `onSnapshot` cache→server geçişini otomatik yönetir. TanStack Query bu okumanın üstünde invalidation katmanı sağlar
18. [ZORUNLU] Yazma Cloud Functions callable üzerinden olduğundan offline'da doğrudan Firestore write yapılmaz — offline callable mutation'ları uygulama seviyesinde queue'lanır ve online olunca replay edilir (ADR-020); queue MMKV/AsyncStorage adapter ile persist edilir
19. [YAPILMAMALI] Firestore cache'ini veya query cache'ini manuel olarak storage'a kopyalamak — SDK persistence + TanStack Query persist plugin kullan
...
33. [ZORUNLU] Auth artefact'ları (ID/refresh token) Firebase SDK tarafından yönetilir; genel storage'da manuel saklanmamalı (ADR-021 ile uyum)
...
- [ ] Storage katmanı doğru seçildi mi? (Firestore offline persistence / SecureStore / MMKV / Query Cache)
...
- ADR-020 — Backend & Data Platform (Firebase, Firestore offline persistence canonical)
- ADR-021 — Authentication Platform (Firebase Auth)
