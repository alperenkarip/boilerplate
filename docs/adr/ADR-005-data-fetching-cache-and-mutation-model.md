...
# 48. Amendment (2026-06-26, ADR-020 / ADR-021): Firebase Backend ile Hizalama

ADR-020 (Backend and Data Platform) ve ADR-021 (Authentication Platform) ile backend Firebase'e kilitlenmiştir. Bu amendment, bu ADR'nin data fetching / cache / mutation modelini Firebase yüzeyine bağlar; mevcut bölümler geçerliliğini korur, aşağıdaki netleştirmeler eklenir (silme yok).

## 48.1. Kaynak (Source of Truth) Firebase'e Bağlanır

- **Okuma kaynağı:** Firestore client SDK (doğrudan client okuma; `getDoc` / `getDocs`), Firestore Security Rules korumalı.
- **Yazma kaynağı:** Cloud Functions callable (`onCall`) / HTTPS (`onRequest`); client doğrudan Firestore'a yazmaz (ADR-020 read/write contract).
- Bu, Bölüm 8'deki "server state'in resmi evi" ilkesini Firebase'e somutlaştırır: server-owned veri yine generic client store'a kopyalanmaz (Bölüm 13).

## 48.2. TanStack Query Rolü (Firebase Bağlamında)

- **Mutation:** TanStack Query mutation lifecycle'ı, yazma yolunda Cloud Functions callable çağrılarını sarar (`FunctionsCallPort` üzerinden). Invalidation, retry ve optimistic update bu katmanda kalır (Bölüm 14-22).
- **Okuma:** Tek seferlik Firestore okumaları TanStack Query ile cache'lenir; query key, stale ve invalidation ownership modeli (Bölüm 9-10, 16) aynen geçerlidir.
- **Realtime:** Firestore `onSnapshot` realtime dinleyicileri TanStack Query cache'i ile senkron tutulur: snapshot güncellemeleri `queryClient.setQueryData` ile cache'e yazılır; UI yine query lifecycle'ını tüketir, kaynağı duplicate etmez.
- Realtime listener'lar logout / user switch sonrası deterministic biçimde sökülür (ADR-021 `LogoutCleanupContract`).

## 48.3. Etki

- Bölüm 11 (Fetch Function) Firebase bağlamında: okuma fetch function Firestore okuması, mutation function callable çağrıdır. Raw Firebase SDK çağrıları yine screen/component'e dağılmaz; data access contract (adapter) katmanında yaşar.
- "Custom fetch + manual cache" reddi (Bölüm 36) Firebase için de geçerlidir: Firestore SDK doğrudan UI'ya dağıtılmaz, query/cache lifecycle korunur.
- React 19 `use(fetch(...))` ile doğrudan Firestore çağrısı yine yasaktır (Bölüm 47.1 mantığı Firestore okuması için de geçerli).
