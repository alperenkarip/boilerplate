...
| 26  | ADR-020 (backend & data platform = Firebase)                  | Zorunlu miras | Override yasak, sadece ADR revision ile                     | Backend & data platform (Firebase), database (Cloud Firestore), server logic (Cloud Functions), storage, scheduled/async jobs zorunlu mirastir. Firebase baska backend ile degistirilemez; client-direct-write, SQL ORM, harici job queue ve rakip BaaS yasaktir. Firebase ici yapilandirma (collection semasi, Security Rules, Cloud Functions tasarimi) proje-ozeldir |
| 27  | ADR-021 (authentication platform = Firebase Auth)             | Zorunlu miras | Override yasak, sadece ADR revision ile                     | Auth platform saf Firebase Auth'tur (client SDK + ID token). Backend session/cookie modeli ve ucuncu taraf auth (Auth0/Clerk/Better Auth) yasaktir; baska auth platformu ile degistirilemez. Auth provider konfigurasyonu (hangi social provider, MFA vb.) proje-ozeldir                                                                                                |
...
| 13  | Backend / Database / Auth platform       | ADR-020, ADR-021, 36                | Zorunlu    | %0 — override yasak          | Firebase ici yapilandirma serbest                 | Backend & data platform (Firebase), database (Cloud Firestore), server logic (Cloud Functions) ve auth (Firebase Auth) hicbir kosulda baska bir platformla degistirilemez. Client-direct-write, backend-managed session/cookie, SQL ORM (Drizzle/Prisma), harici job queue (Inngest/BullMQ), rakip BaaS (Supabase/Neon) ve harici auth (Better Auth/Auth0/Clerk) yasaktir. Proje Firebase ici yapilandirmayi (collection semasi, Security Rules, Cloud Functions tasarimi, auth provider secimi) serbestce belirler; bu serbestlik platform secimini gevsetmez. |
