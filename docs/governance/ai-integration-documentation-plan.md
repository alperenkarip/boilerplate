...
- `/moai plan` → EARS formatında SPEC üretir → `/moai run SPEC-XXX` → Claude Code implement eder.
...
Bu kararlar ADR-001→011, 36, 37, 38 ile kilitlenmiştir.
...
Talimat dosyaları (CLAUDE.md, AGENTS.md) canonical karar katmanını (ADR-001→011, 36, 37, 38) doğru yansıtmak zorundadır. Canonical katmanda değişiklik olursa talimat dosyaları da güncellenir. Bu güncelleme contribution guide kapsamında zorunlu adımdır.
...
6. Araçlar Arası Etkileşim Modeli
   6.1. MoAI-ADK → Claude Code: SPEC-to-Implementation
        - /moai plan → SPEC üret
        - /moai run SPEC-XXX → Claude Code implement eder
...
9. Talimat Dosyaları Yönetimi
   ...
   9.2. Tutarlılık kuralı
        Tüm talimat dosyaları canonical kararları (ADR-001→011, 36, 37, 38)
...
7. Adlandırma Kuralları
   7.1. SPEC dosyaları: SPEC-{NNN}.md (.moai/specs/ altında)
...
Boilerplate güncellendi (upstream)
  │
  ├─ Canonical katman değişikliği var mı? (ADR, 36, 37, 38)
