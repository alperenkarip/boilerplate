...
| `build_site`                | Ekranlari route'lara eslestirerek site scaffold'u olusturur |

**Stitch Skill Ekosistemi (Claude Code icinde):**

Stitch, 7 resmi agent skill'i sunar. Her skill belirli bir pipeline adimini otomatiklestirir:

| Skill              | Kurulum                                                                    | Islem                                                                    |
| ------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| `design-md`        | `npx skills add google-labs-code/stitch-skills --skill design-md --global` | Stitch projesini analiz ederek DESIGN.md sentezler                       |
| `react:components` | `npx skills add google-labs-code/stitch-skills --skill react:components`   | Stitch ekranlarini React/JSX + Tailwind component'lere donusturur        |
| `enhance-prompt`   | `npx skills add google-labs-code/stitch-skills --skill enhance-prompt`     | Tasarim prompt'unu zenginlestirir (platform, renk, grid detaylari ekler) |
| `stitch-design`    | `npx skills add google-labs-code/stitch-skills --skill stitch-design`      | Canvas uzerinde dogal dille iterasyon yapar                              |
| `stitch-loop`      | `npx skills add google-labs-code/stitch-skills --skill stitch-loop`        | Surekli tasarim-kod dongusu (tam otomasyon)                              |
| `stitch-to-figma`  | Stitch arayuzunde tek tikla                                                | Canvas'i Figma formatina aktarir                                         |
| `stitch-site`      | `npx skills add google-labs-code/stitch-skills --skill stitch-site`        | Ekranlari route yapisiyla birlikte scaffold eder                         |

Detayli skill dokumantasyonu, kullanim matrisi ve governance iliskisi icin: `46-stitch-pipeline-spec.md` bolum 12.
...
**Stitch 2.0 Yetenekleri (Mart 2026):**

- Voice Canvas: sesli komutlarla tasarim iterasyonu
- Vibe Design: hissiyat tabanli tasarim yonlendirme
- Multi-screen generation: tek oturumda 5 ekrana kadar tutarli tasarim
- AI-native Infinite Canvas: sketch'ten prototipe donusum
- Interaktif prototipleme: ekranlar arasi gecis testi
- Detay: `46-stitch-pipeline-spec.md` bolum 13.

**Kapasite:** Aylik 350 (Standard) + 200 (Pro) + 50 (Experimental) = 600 uretim/ay. Tamamen ucretsiz (Nisan 2026). Detay: `46-stitch-pipeline-spec.md` bolum 14.

**React Native Sinirliligi:** Stitch'in dogrudan React Native export'u yoktur. Tailwind CSS ciktisi NativeWind'e donusturulur. Detay: `46-stitch-pipeline-spec.md` bolum 15.
...
- Dogrudan React Native export yoktur; NativeWind donusum zinciri uygulanir.
...
```
Stitch'te tasarim olustur (dogal dil, gorsel girdi, Voice Canvas veya Vibe Design)
     ↓
design-md skill ile DESIGN.md sentezle (veya manuel export)
     ↓
Stitch MCP ile canli tasarim verisi cek (extract_design_context, fetch_screen_code)
     ↓
react:components skill ile React component uret
     ↓
Claude Code ile token eslestirme + governance refinement
     ↓
NativeWind donusumu (mobile hedefli projelerde)
```

**Baglanti mekanizmasi:** MCP protokolu + skill ekosistemi. Claude Code, Stitch MCP uzerinden canli tasarim verisine erisir. `design-md` skill'i DESIGN.md sentezler, `react:components` skill'i React component'lere donusturur, `stitch-loop` skill'i tam donguyu otomatiklestirir.

**Veri akisi:** Stitch canvas → DESIGN.md → MCP → Claude Code → Token eslestirme (22) → React component → NativeWind donusumu (mobile)

**Detayli pipeline dokumantasyonu:** `46-stitch-pipeline-spec.md`
...
Yeni bir ekran veya component icin tam pipeline:

```
1. Stitch'te tasarim olustur (enhance-prompt ile zenginlestirilmis briefing)
2. design-md skill ile DESIGN.md sentezle
3. Token eslestirme: DESIGN.md → 22-design-tokens-spec.md katmanlariyla dogrulama
4. /moai plan ile SPEC uret (gerekiyorsa)
5. react:components skill ile React component uret
6. Claude Code ile governance refinement (23, 26, D-DSY, D-STY, D-UIX)
7. NativeWind donusumu (mobile hedef varsa)
8. PR ac
9. @codex review
10. Insan onayi + merge
```

Detayli pipeline adim aciklamalari: `46-stitch-pipeline-spec.md` bolum 3.
...
```
1. Stitch'te tasarim olustur (enhance-prompt ile briefing, Vibe Design veya Voice Canvas)
2. design-md skill ile DESIGN.md sentezle
3. DESIGN.md → 22-design-tokens-spec.md ile token tutarlilik dogrula
4. react:components skill ile React component uret
5. Claude Code ile governance refinement (token eslestirme, a11y, platform adaptation)
6. NativeWind donusumu (mobile hedef varsa — 46 bolum 15)
7. Component'i Tailwind CSS 4.x (ADR-007) ile style et
8. Test yaz (Testing Library — ADR-008)
9. PR ac
10. @codex review
11. Insan review + merge
```

Detay: `46-stitch-pipeline-spec.md` bolum 3 ve 16 (briefing rehberi).
...
```
1. Stitch'te tasarim degisikligini yap (stitch-design skill ile iterasyon)
2. design-md skill ile DESIGN.md yeniden sentezle
3. 22-design-tokens-spec.md ile token tutarlilik dogrula
4. Claude Code ile token sync (packages/design-tokens/ → tailwind.config → NativeWind)
5. Etkilenen component'leri guncelle
6. Gorsel regresyon testi (Playwright 1.58.x — ADR-008)
7. Design-to-code diff raporu olustur (46 bolum 11)
8. PR ac
9. @codex review
10. Insan review + merge
```
