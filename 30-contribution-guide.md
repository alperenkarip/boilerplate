...
Native build yapmadan hızlı başlamak için fiziksel cihazına Expo Go uygulamasını (App Store / Google Play) kur. `pnpm dev:mobile` ile dev server'ı başlat, terminaldeki QR kodu Expo Go ile tara. Bu yöntem native modül gerektirmeyen geliştirmeler için yeterlidir. Native modül gerektiğinde development build'e geçiş yapılmalıdır (`npx expo prebuild`).
